...
# 환경 변수 직접 설정 (Streamlit Cloud에서 사용)
...
# OpenAI API 키 설정 - API 키 매개변수 없이 초기화
try:
    if not api_key:
        st.warning("OpenAI API 키가 설정되지 않았습니다. 환경 변수나 .env 파일을 확인해주세요.")
        llm = None
    else:
        # 새로운 방식으로 초기화 시도
        llm = ChatOpenAI(
...
def process_message(user_input: str):
    try:
        ...
        return f"죄송해요, 대화 처리 중 문제가 발생했어요. 다시 시도해 주세요."
...
if prompt := st.chat_input("메시지를 입력하세요..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("ChaCha가 생각하고 있어요..."):
            try:
                if llm is None:
                    response = "API 설정 오류로 응답할 수 없습니다. API 키를 확인해주세요."
                else:
